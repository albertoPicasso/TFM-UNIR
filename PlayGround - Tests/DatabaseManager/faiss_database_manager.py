...
class Faiss_database_manager(Base_database_manager):
    """
    ...
    def create_and_save_embeddings_with_timer (self, documents: list[list[Document]], database_name:str = "test1.db") -> None:
        ...
        save_path = "LLM_testing/times/databases/Faiss_paraphrase-multilingual-mpnet-base-v2.csv"
    ...
    def get_context_with_stats(self, database_name: str, query_text: str, k: int = 10) -> list:
        ...
        #Its safe because is loading our databases, NEVER LOAD AN EXTERNAL DATABASE WITH THIS METHOD(Pickle files)
        start_time = time.time()

        vector_store= FAISS.load_local(folder_path= database_path, embeddings=self.embedding_model, allow_dangerous_deserialization = True)
        ...
        save_path = "LLM_testing/Times/retrieval/Faiss.csv"
    ...
    def get_context(self, database_name: str, query_text: str, k: int = 10) -> list:
        """
        Retrieve the top K most relevant documents from the vector database.
